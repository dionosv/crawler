  ...
  return new Promise((resolve) => {
    if (isMainThread) {
      ...
      for (let i = 0; i < numWorkers; i++) {
        const worker = new Worker('./jawapos/worker.js', { workerData: { link: links[i], nomor: i + 1 } });
...
const runjawapos = async (berapaindex = 1, berapatabsize = 5) => {
...
module.exports = runjawapos;


//note ini tinggal masukin angka di runjawapos(disini), angka tsb per berapa indeks yg mau di scraping, 1 indeks ada 20 link, jadi nanti jalan berapa kali indeks

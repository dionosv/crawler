...
async function crawl(loop = 1) {
  try {
    var baselink = "https://www.sindonews.com/topic/3627/pilpres-2024/" ;

    if(loop > 1){
      loop = 24 * (loop-1)
      baselink = "https://www.sindonews.com/topic/3627/pilpres-2024/" + loop;
    }
    ...
    const berita = await page.evaluate(() => {
      const links = Array.from(document.querySelectorAll('a.desc-kanal.mb5.nasional-title')).map(element => element.href);
      return links;
    });
    await browser.close();
    return berita
